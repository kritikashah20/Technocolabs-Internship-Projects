...
def main():
    ...
    LIMIT_BAL = st.sidebar.text_input(
        "Amount of Credit provided including individual & family credit (in New Taiwanese (NT) dollar)")
    ...
    if st.button("Predict"):
        ...
        if prediction == 1:
            ...
            st.write("Probability of account being defaulted is {}%.".format(prediction_proba[:, 1][0] * 100))

        else:
            st.write("The account will not be defaulted")
            st.subheader('Prediction Probability')
            st.write("Probability of account not being defaulted is {}%.".format(prediction_proba[:, 0][0] * 100))
